...
### 1. Прямой доступ к `woo-mcp`

`op-assistant` должен ходить прямо в `woo-mcp`, без MAG.

### 2. Approval gate вызывается перед фактическим action execution

Для текущего стека это значит:

- generated tool вызывает transport
- transport использует generic approval gate
- transport передаёт в gate `actionName`, `args`, `context`, `execute`
- реальный MCP call происходит только внутри `execute`

### 3. Жёсткая runtime-гарантия

Должен остаться один допустимый путь выполнения Woo action:

`tool -> transport public method -> approval gate -> raw execution`
...
1. Переключить `op-assistant` на прямой вызов `woo-mcp`.
