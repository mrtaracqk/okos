...
`catalog-agent` больше не передает воркеру свободный `task/context`.

Теперь каждый handoff строится как локальная постановка из пяти блоков:
...
- `Что сделать` описывает только локальное действие в зоне ответственности конкретного воркера;
- `Данные для выполнения` содержит все нужные факты и входные значения сразу;
- `Почему` объясняет только непосредственную необходимость текущего шага;
- `Что вернуть` жестко фиксирует формат результата;
- общая цель и следующие шаги плана в handoff не передаются.
...
Финал worker-а теперь тоже не свободный текст.

У каждого worker-а есть служебный финальный инструмент:

- `report_worker_result`

Через него worker обязан вернуть структурированный итог:

- `status`: `completed | blocked | failed`
- `data`: только факты и результаты
- `missingData`: только реальные блокеры
- `note`: короткая фактическая заметка

После вызова `report_worker_result` loop worker-а завершается.

Важно:

- worker не знает про транспорт как концепцию;
- для worker-а существуют только tool names, schema и tool results;
- выполнение Woo tools идёт через `src/services/wooToolExecutor.ts` и `src/services/wooClient.ts`.
...
Worker tools определены в коде, без генерации из snapshot.

Источник:

- `src/agents/catalog/specialists/shared/wooTools/`: фабрика `createWooTool`, наборы product/category/attribute/variation tools.

Runtime:

- `src/services/wooClient.ts`: клиент Woo REST API из env (`WOOCOMMERCE_REST_BASE_URL`, опционально consumer key/secret).
- `src/services/wooToolExecutor.ts`: вызов инструмента, нормализация результата, approval при `requiresApproval: true`, трассировка.

Смысл:

- каждый worker получает только свой набор tools из соответствующего `*Tools.ts`;
- выполнение идёт через in-app Woo REST SDK, без MCP;
- мутирующие операции могут объявлять `requiresApproval: true` и проходить через Telegram approval.
...
- worker-ы это самостоятельные специалисты со своими Woo tools из `wooTools/*`;
- выполнение Woo идёт через in-app SDK (wooClient + wooToolExecutor), в промпты не попадает;
