...
export const productWorkerSpec = {
  ...
  worker: {
    ...
    toolUsage: [
      ...
      'products_list и products_read возвращают сокращённую проекцию товара, нам не нужны все возможности для наших сценариев.',
  ...
  foreman: {
    routingSummary: [
      'Родительская карточка (simple/variable), categories и attributes/default_attributes на родителе; read/list в категориях, глобальных атрибутах/term-ах и variation того же товара только для prerequisite.',
      'Создание категорий, taxonomy или конкретных variation — category-worker, attribute-worker или variation-worker; после prerequisite шаг возвращается в product-worker.',
    ],
    consultationSummary: [
